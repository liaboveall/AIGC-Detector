"""Compare aligned vNext predictions with generator-macro robustness metrics."""
...
def load_aligned(baseline_path: Path, candidate_path: Path, manifest_path: Path) -> pd.DataFrame:
    ...
    return merged[
        keys + static + ["content_group", "baseline_probability", "candidate_probability"]
    ]
...
def summarise(
    ...
    for dataset in sorted(frame["dataset"].unique()):
        ...
        generators = sorted(
            dataset_frame.loc[dataset_frame["label"] == 1, "source_class"].unique()
        )
...
def main() -> None:
    ...
    gates = {
        ...
        "bootstrap_ci_positive": bootstrap is None or bootstrap["ci95_lower"] > 0.0,
    }
    payload = {
    ...
    print(json.dumps({"delta": delta, "gates": gates, "accepted": payload["accepted"]}, indent=2))
